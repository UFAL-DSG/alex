...
class MarkSubject(Block):
    ...
    def process_ttree(self, ttree):
        ...
        # filter out oblique casus and TWHEN etc. expressions, e.g. "minulé pondělí" etc.
        # and create a set of viable a-nodes
        nominatives = set([cand.lex_anode for cand in ttree.get_descendants()
                       if cand.formeme in ['n:1', 'drop'] and not cand.functor.startswith('T')])
        # find all verbs and mark subjects for each of them
        for tnode in filter(lambda t: re.match(r'^v.+(fin|rc)$', t.formeme), ttree.get_descendants()):
            if tnode.lex_anode: 
                asubj = self.__find_subject(tnode.lex_anode, nominatives)
                if asubj is not None: asubj.afun = 'Sb'
    ...
    def __find_subject(self, anode, nominatives):
        ...
        candidates = list(reversed(anode.get_echildren(preceding_only=True))) + anode.get_echildren(following_only=True)
        # discard those which are not "proper" nominatives, return if nothing remains
        candidates = filter(lambda a: a in nominatives, candidates)
        # filter copula verb candidates - demonstrative pronouns
        if anode.lemma == 'být':
            candidates = filter(lambda a: a.lemma not in ['ten', 'tento'], candidates)
