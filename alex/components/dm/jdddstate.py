...
class JasonDeterministicDiscriminativeDialogueStateException(DialogueStateException):
    pass

class JasonDeterministicDiscriminativeDialogueState(DialogueState):
    """This is a implementation of a dialogue state and its update based on the 
    deterministic discriminative dialogue state.
    
    It uses confidence in the best DA in its updates.
    ...
    def __init__(self, cfg):
        super(JasonDeterministicDiscriminativeDialogueState, self).__init__(cfg)
    ...
    def __str__(self):
        ...
        s = []
        s.append("JasonDDDState - Dialogue state content:")
        s.append("")
    ...
    def update(self, user_da, last_system_da):
        ...
        if isinstance(user_da, DialogueAct):
            ...
            raise JasonDialogueManagerException("Unsupported input for the dialogue manager.")


        if self.cfg['DM']['Jason']['debug']:
            self.cfg['Logging']['system_logger'].debug('JasonDDDState Dialogue Act in: %s' % da)
