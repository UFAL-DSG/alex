...
class MulticlassLogisticRegression:
    """Implementation of multiclass logistic regression. It provides two ways of training:
      1) normal gradient descend 
      2) natural gradient descend
      
      Note that the natural gradient descend is much faster, however, it is also much more memory demanding (its memory complexity is O(N^2)). 
    """
    ...
    def w(self, phis):
        ...
        for phi in phis:
            ...
            if prod < -1e16 or prod > 1e16:
                print "MulticlassLogisticRegression::w - too small/large numbers"
            w.append(prod)
        return np.array(w)
        
    def ew(self, phis):
        """Compute exponentiate of the product of the parameters and the features."""
        return np.exp(self.w(phis))
        
    def Pcf(self, cls, phis):  
        """Returns probability of the cls class given the phis (class) features."""
    ...
    def gradLogLikelihood(self, examples):
        """Compute the gradient of the likelihood of the examples and
            at the same time also compute the likelihood.
        
            Normalize both the gradient and the likelihood for the 
    ...
    def update(self, examples, alg = "plain", step_size = 1.0, regularization = 0.0):
        """The theta parameters get updated by either:
          ...
          2) alg       - optimization algorithm. In both cases, the likelihood of the examples is computed.
...
# The next function is just for inspiration when using the MLR class. It should be moved to an independent test class.
