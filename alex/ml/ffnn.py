...
class TheanoFFNN(object):
    ...
    def __init__(self, n_inputs, n_hidden_units, n_hidden_layers, n_outputs, hidden_activation = 'tanh', weight_l2 = 1e-6):
        ...
        true_y = T.iscalar('true_y')  # The desired output vector.
        loss = -T.log(y[0][true_y])  # Negative log-likelihood.
    ...
    def sgrad_minibatch(self, data):
        ...
        for t_x, t_y in zip(data[0], data[1]):
...
#        print 'cudasolve> Iterations required on GPU:', i
#        r = b - culinalg.dot(A,x)
#        r_norm = culinalg.norm(r)
        
#        print 'cudasolve> |rs|', math.sqrt(rsnew)
#        print 'cudasolve> |r|', r_norm, '|b|', b_norm
    ...
    def ngrad_minibatch(self, data_x, data_y, alpha = 1e-3):
        ...
        # Go through the data, compute gradient at each point and accumulate it.
#        l_cg = []
#        for t_x, t_y in zip(data_x, data_y):
#            t_x = t_x.reshape((1, t_x.shape[0]))
#            t_y = t_y.reshape((1,))
...
#            t_cg += cg
        ...
        gpu_t_cg = gpuarray.to_gpu(t_cg)
        gpu_l_cg = gpuarray.to_gpu(l_cg)
        # explict representation of FIM is for ineffcient if the size of a minibatch is significantlu smaller compared to the number of the 
        # model parameters  
#        gpu_t_F = culinalg.dot(gpu_l_cg, gpu_l_cg, transa='T')
#        gpu_t_F /= len(data_x)
#        gpu_identity = gpuarray.to_gpu(alpha*np.identity(n_F))
