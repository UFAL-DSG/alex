...
for s in train $TEST_SETS ; do
    ...
    # Note --fake -> NO CMVN steps/compute_cmvn_stats.sh --fake $WORK/local/$s \
...
for s in $TEST_SETS ; do
  for lm in $LMs ; do
    ...
    steps/decode.sh --scoring-opts "--min-lmw $min_lmw --max-lmw $max_lmw" \
      --run-cmn $cmn --config common/decode.conf --nj $njobs --cmd "$decode_cmd" \
    ...
    steps/decode.sh --scoring-opts "--min-lmw $min_lmw --max-lmw $max_lmw" \
      --run-cmn $cmn --config common/decode.conf --iter 4 --nj $njobs --cmd "$decode_cmd" \
