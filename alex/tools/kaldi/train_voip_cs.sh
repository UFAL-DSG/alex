...
utils/check.sh local/data_split.sh --every_n $EVERY_N \
    $DATA_ROOT $WORK/local "$LM_paths" "$TEST_SETS" || exit 1

utils/check.sh local/create_LMs.sh \
    --train_text $WORK/local/train/trans.txt \
    --arpa-paths "$LM_paths" --lm-names "$LM_names" \
    $WORK/local/lm || exit 1

utils/check.sh local/prepare_cs_transcription.sh $WORK/local/lm $WORK/local/dict || exit 1

utils/check.sh local/create_phone_lists.sh $WORK/local/dict || exit 1

utils/check.sh utils/prepare_lang.sh $WORK/local/dict '_SIL_' $WORK/local/lang $WORK/lang || exit 1

utils/check.sh local/create_G.sh $WORK/lang "$LM_names" $WORK/local/lm $WORK/local/dict/lexicon.txt || exit 1

echo "Create MFCC features and storing them (Could be large)."
for s in train $TEST_SETS ; do
    utils/check.sh steps/make_mfcc.sh --mfcc-config common/mfcc.conf --cmd "$train_cmd" \
      --nj $njobs $WORK/local/$s $EXP/make_mfcc/$s $WORK/mfcc || exit 1
    # Note --fake -> NO CMVN
    utils/check.sh steps/compute_cmvn_stats.sh $fake $WORK/local/$s \
...
utils/check.sh steps/train_mono.sh  --nj $njobs --cmd "$train_cmd" $WORK/train $WORK/lang $EXP/mono || exit 1
...
utils/check.sh steps/train_deltas.sh  --cmd "$train_cmd" \
...
utils/check.sh steps/make_denlats.sh  --nj $njobs --cmd "$train_cmd" \
...
utils/check.sh steps/train_mmi.sh  --boost ${train_mmi_boost} $WORK/train $WORK/lang \
...
for lm in $LM_names ; do
  lm=`basename "$lm"`
  # utils/check.sh utils/mkgraph.sh --mono $WORK/lang_${lm} $EXP/mono $EXP/mono/graph_${lm} || exit 1
  # utils/check.sh utils/mkgraph.sh $WORK/lang_${lm} $EXP/tri1 $EXP/tri1/graph_${lm} || exit 1
  utils/check.sh utils/mkgraph.sh $WORK/lang_${lm} $EXP/tri2b $EXP/tri2b/graph_${lm} || exit 1
done
...
for s in $TEST_SETS ; do
  for lm in $LM_names ; do
    ...
    # utils/check.sh steps/decode.sh --scoring-opts "--min-lmw $min_lmw --max-lmw $max_lmw" \
...
utils/check.sh local/results.py $EXP | tee $EXP/results.log

utils/check.sh local/export_models.sh $TGT_MODELS $EXP $WORK/lang
