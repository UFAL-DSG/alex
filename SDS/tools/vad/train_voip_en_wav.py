...
def train_speech_gmm():
  vta_speech = MLFMFCCOnlineAlignedArray(filter='speech')
  ...
  for i in range(n_mixies):
    ...
    print "-"*120
  gmm_speech.save_model('vad_speech_wav.gmm')
  return

def train_sil_gmm():
  vta_sil = MLFMFCCOnlineAlignedArray(filter='sil')
  ...
  for i in range(n_mixies):
    ...
    print "-"*120
  gmm_sil.save_model('vad_sil_wav.gmm')
